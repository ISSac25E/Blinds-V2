...
// MQTT Server Setup:
#define BLINDS_MQTT_SERVER_IP "10.0.0.100"
...
// Stepper Speed Setup:
#define STEPPER_STEPS_PER_SEC 555           // Steps per second
#define STEPPER_CALIBRATE_STEPS_PER_SEC 555 // Steps per second When Homeing/Calibrating (Usually slower than normal speed)
#define STEPPER_STEPS_DIVISION 40           // How many times MQTT Position Message will be Multiplied by
#define BLINDS_MAX_CALIBRATE_STEPS 1000     // Max Calibration Steps (Max number of Steps Stepper will move while calibrating)
#define BLINDS_DIR FOWARD                   // Normal Direction of Blinds(FOWARD, BACKWARD) *default is FOWARD
#define BLINDS_CALIBRATE_DIR FOWARD         // Direction of Calibration(FOWARD, BACKWARD) *was BACKWARD
// Stepper Pins Setup:
#define STEPPER_STEP_PIN D8                      // Pin For Stepper Step Pin
#define STEPPER_DIR_PIN D7                       // Pin For Stepper Direction Pin
#define STEPPER_EN_PINS {D0, D5, D4, D3, D2, D1} // All Enable Pins for Steppers. This is where you configure how many steppers you need.
    ...
    for (uint8_t X = 0; X < StepperCount; X++)
    {
      if (!CalibrateStepper[X])
      {
        if (StepperCurrentVal[X] < StepperTargetVal[X])
        {
          ...
          StepperCurrentVal[X]++;
          digitalWrite(STEPPER_DIR_PIN, BLINDS_DIR);
    ...
    if (CalibrateStepper[X])
    {
      ...
      else
      {
        if (StepperDrive.Ready())
        {
          ...
          for (uint8_t Y = 0; Y < StepperCount; Y++)
            digitalWrite(StepperPins[Y], HIGH);
          digitalWrite(STEPPER_DIR_PIN, BLINDS_CALIBRATE_DIR);
          digitalWrite(StepperPins[X], LOW);
